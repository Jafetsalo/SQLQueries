...
@Cantidad int
as 
Insert into tblMaterial Values (@Nombre_Material,@Valor_Material,@Año,@CodTipo_Material,@Cantidad)
...
La base de datos de la biblioteca contiene varias tablas para gestionar la información de los usuarios, materiales, préstamos, reservas y devoluciones. 
...
La tabla 'tblTipo_Usuario' almacena los diferentes tipos de usuarios que pueden acceder a la biblioteca. La tabla 'tblUsuario' contiene la información personal de los usuarios registrados, como su nombre, dirección y número de teléfono. La tabla 'tblDependencia' almacena información sobre las dependencias de la biblioteca, como las áreas de estudio y las salas de conferencias. La tabla 'tblPertenece' es una tabla de relación entre las tablas 'tblUsuario' y 'tblDependencia' para identificar qué usuario pertenece a qué dependencia.

La tabla 'tblMaterial' contiene información sobre los diferentes tipos de materiales disponibles en la biblioteca, como libros, revistas y discos compactos. La tabla 'tblEjemplar' contiene información específica de cada ejemplar de material, como su número de inventario y la fecha de adquisición. La tabla 'tblReserva' almacena información sobre las reservas realizadas por los usuarios para los materiales de la biblioteca.

La tabla 'tblPrestamo' registra los préstamos de materiales a los usuarios y la fecha de devolución prevista. La tabla 'tblDevolucion' registra la devolución real de los materiales prestados y la fecha en que se realizó la devolución. Finalmente, la tabla 'tblTipo_Material' almacena información sobre los diferentes tipos de materiales, como su categoría y subcategoría. En resumen, esta base de datos es una herramienta valiosa para la gestión de la biblioteca, proporcionando una visión completa de la información necesaria para llevar a cabo sus operaciones diarias.
