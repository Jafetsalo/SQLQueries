...
CREATE table worker
(
Worker_ID int not null, /* NOT NULL IS REDUNDANT HERE A Primary Key is always not null*/
Worker_National_ID int,
Primary Key (Worker_ID),

Foreign Key (Worker_National_ID) references tblusuario (Cedula) ON UPDATE CASCADE ON DELETE SET NULL
)

Select Nombre as NM, Cedula as PP from tblusuario where Nombre in (SELECT Nombre FROM tblusuario)
